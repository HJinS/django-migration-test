...
from .migration_squash_checker import SquashTestSample
...
class LoaderTests(TestCase):
    ...
    def test_migration_names_set_order_succeeds(self):
        ...
        checker = SquashTestSample()
        sub_checker = multiprocessing.Process(
            target=checker.test_migration_names_set_order_succeeds,
            args=(data_queue,)
        )
        sub_checker.start()
        sub_checker.join()
        result_dict = data_queue.get_nowait()

        error_raised = result_dict.get(
            "CircularDependencyErrorRaised", {}
        ).get("value")
        msg = result_dict.get(
            "CircularDependencyErrorRaised", {}
        ).get("msg")
        ...
        imported_order = result_dict.get(
            "migrationImportOrderValue", {}
        ).get("imported")
        expected_order = result_dict.get(
            "migrationImportOrderValue", {}
        ).get("expected")
